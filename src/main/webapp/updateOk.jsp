...
<%@page import="kr.human.second.vo.UpFileVO"%>
...
<body>
	...
	<%
		...
		switch(m){
		case 1: // 저장
			NoticeServiceImpl.getInstance().insert(NoticeVO);
			query = "<script>sendPost(\"index.jsp\",{\"p\":1,\"b\":"+b+",\"s\":"+s+"});</script>";
			break;
		case 2: // 수정
			NoticeServiceImpl.getInstance().update(NoticeVO, application.getRealPath("/upload/"), delfile);
			query = "<script>sendPost(\"view.jsp\",{\"p\":1,\"b\":"+b+",\"s\":"+s+",\"idx\":"+idx+"});</script>";
			break;
		case 3: // 삭제
			NoticeServiceImpl.getInstance().delete(NoticeVO, application.getRealPath("/upload/"));
